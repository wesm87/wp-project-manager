...
var upperSnakeCase = lodash_fp.compose(lodash_fp.replace(/ /g, '_'), lodash_fp.startCase);
...
var Project = function () {
  ...
  _createClass(Project, null, [{
    ...
    value: function parseConfig(config) {
      ...
      parsed.plugin.id = lodash_fp.snakeCase(parsed.plugin.name);
      ...
      parsed.theme.id = lodash_fp.snakeCase(parsed.theme.name);
    ...
    value: function ensureProjectConfig(config) {
      ...
      if (!parsed.project.title && parsed.project.slug) {
        parsed.project.title = lodash_fp.startCase(parsed.project.slug);
      }

      if (!parsed.project.slug && parsed.project.title) {
        parsed.project.slug = lodash_fp.kebabCase(parsed.project.title);
      }
    ...
    value: function ensurePluginConfig(config) {
      ...
      if (!parsed.plugin.name) {
        if (parsed.plugin.slug) {
          parsed.plugin.name = lodash_fp.startCase(parsed.plugin.slug);
        } else {
      ...
      if (!parsed.plugin.slug) {
        parsed.plugin.slug = lodash_fp.kebabCase(parsed.plugin.name);
      }
    ...
    value: function ensureThemeConfig(config) {
      ...
      if (!parsed.theme.name) {
        if (parsed.theme.slug) {
          parsed.theme.name = lodash_fp.startCase(parsed.theme.slug);
        } else {
...
var isTest = lodash_fp.isEqual('node-test');
var isDev = lodash_fp.isEqual('develop');
var isProd = lodash_fp.isEqual('production');
...
var Scaffold = function (_Project) {
  ...
  _createClass(Scaffold, null, [{
    ...
    value: function getBasePath() {
      ...
      var pathKey = lodash_fp.camelCase(type);
